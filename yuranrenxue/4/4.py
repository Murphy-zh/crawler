...
def request(page):
    ...
    url = "http://match.yuanrenxue.com/api/match/4?page={page}".format(page=page)
    
    resp = requests.get(url=url,headers=headers)
    info = resp.json().get("info")
    return info,resp.json().get("key"),resp.json().get("value")


def get_md5_class(key,value):
     # btoa()  js中将普通字符串编码成base64，对应base64.b64encode() 
     # /=/g 全部匹配，否则只匹配一个
     str_class = base64.b64encode((key + value).encode()).replace(b'=', b'')
     md5_class = hashlib.md5(str_class).hexdigest()
     return md5_class
     
def get_img(info,md5_class):
    sum_per = 0
    content = etree.HTML(info)
    tds = content.xpath("//td")
    for td in tds:
        num1_list = []
        imgs = td.xpath("./img")
        for img in imgs:
            src = img.xpath("./@src")[0]
            cls = img.xpath("./@class")[0].replace("img_number ","")
            style = img.xpath("./@style")[0].replace("left:","").replace("px","")
            if cls != md5_class:
                num = img_dict[src]
                img_list =[num,style]
                num1_list.append(img_list)

        new_num=num1_list.copy()
        for i,tup in enumerate(num1_list):
            # print(i,tup)
            index=int(float(tup[1])/11.5)
            if index!=0:
                # print(index+i,index)
                new_num[i+index]=num1_list[i]
...
if __name__ == "__main__":
    ...
    for page in range(1,6):
        info,key,value = request(page)
        md5_class = get_md5_class(key,value)
        sum_per = get_img(info,md5_class)
        sum_all += sum_per
    print(sum_all)
